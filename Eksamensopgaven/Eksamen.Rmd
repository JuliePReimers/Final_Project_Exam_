...
data_cph <- datasæt_mediestream %>% filter(lplace == "København")


```

# To find the newspapers published in Schleswig, we have chosen to analyse papers from Ribe, Haderslev and Tønder. 

```{r}

data_slw <- filter(datasæt_mediestream, lplace == "Tønder" | lplace == "Haderslev" | lplace == "Ribe")
...
data_slw%>% 
  mutate(m = month(timestamp)) -> data_slw

```

```{r}
data_tokens <- data_slw %>% 
...
```{r}
danish_stopwords <- "data/Danish_stopwords.csv"
...
slw_tidy %>%
  ...
  left_join(total_words, by = "m") -> slw_counts
...
```{r}
slw_counts %>% 
  bind_tf_idf(word, m, n) -> slw_tfidf
...
slw_tfidf %>%
  ...
  labs(
      title = "Schleswig Newspapers: most important words pr. month",
...
### Udled teksten fra "full_text" kolonnen:
```{r get-document}
ipcc_path <- here(slw_tidy)
